    ...
    return(
      ...
      {this.props.survey.questionList.map((q, i)=>{
        switch(q.responseType){
            case "Radio":{
               return <RadioQuestion question={q} index={i} onClick={(resp)=>{this.handleClickRadio(resp,i)}} />
            }
            case "Checkbox":{
               return <CheckBoxQuestion question={q} index={i} onClick={(resp)=>{this.handleClickCheck(resp,i)}}/>
            }
            case "Freeform":{
                return (
                <FormGroup>
                <Label for="exampleText">{q.question}</Label>
                <Input type="textarea" name="text" id="exampleText" onChange={(onChange)=>this.handleInputChange(onChange,i)}/>
              </FormGroup>)
            }
        }
      })
